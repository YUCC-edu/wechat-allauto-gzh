...
2. **确认机制**：在执行破坏性操作（如删除菜单、删除已发布文章）或正式发布操作（`publish` -> `submit`）前，建议先向用户进行二次确认。
3. **微信图片防盗链**：在前端预览 iframe 中，如果用户粘贴了外部图片，可能会因为防盗链无法显示。我们在 `<img>` 标签中已经默认添加了 `referrerPolicy="no-referrer"` 来尽量规避此问题，修改图片渲染逻辑时切勿遗漏此属性。
4. **代码块高亮**：目前代码块的处理在 `WeChatHTMLConverter.ts` 的 `processCodeBlocks` 中。如果用户需要更复杂的语法高亮，你需要确保高亮样式被转换为**内联样式**，否则微信后台会将其过滤掉。
